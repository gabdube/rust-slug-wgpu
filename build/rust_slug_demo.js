...
(function (MessageType) {
    MessageType[MessageType["UpdateVertexBuffer"] = 0] = "UpdateVertexBuffer";
    MessageType[MessageType["UpdateFontAtlas"] = 1] = "UpdateFontAtlas";
    MessageType[MessageType["DrawText"] = 2] = "DrawText";
    MessageType[MessageType["InvalidateText"] = 3] = "InvalidateText";
...
class UpdateFontAtlasParams {
    ...
    get curves_indices_offset() {
        return this.view.getUint32(this.offset + 20, true);
    }
    get curves_indices_size() {
...
class Message {
    ...
    get update_font_atlas() {
        return new UpdateFontAtlasParams(this.view, this.offset + 4);
    }
}
function read_output_messages(memory, index_ptr) {
...
function align(value, n) {
    return (value + n - 1) & ~(n - 1);
}
function computeMvp(user_state, out) {
    const scaleX = 2.0 / user_state.view_width;
    const scaleY = -2 / user_state.view_height;
...
async function loadDemo(assets, resources, user_state, stats) {
    const module = await import(WASM_PATH);
    const init_output = await module.default();
    ...
    const fontData = await Promise.all(assets.fonts.map(async (font) => ({ name: font.name, data: (await (await fetch(font.path)).arrayBuffer()) })));
    if (fontData) {
...
function initUserControls(demo) {
    let isPressed = false;
    let lastX = 0;
    let lastY = 0;
    ...
    demo.canvas.addEventListener("drop", (e) => {
        ...
        if (fontExtensions.includes(extension)) {
            ...
            reader.onload = (event) => {
                ...
                if (!default_text_instance || default_text_instance.id === null) {
                    return;
                }
                const add_text_start = performance.now();
